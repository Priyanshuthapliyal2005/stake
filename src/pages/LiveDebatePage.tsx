...
import { SpeechRecognitionControls } from '../components/SpeechRecognitionControls';
...
export function LiveDebatePage() {
  ...
  // Caption controls - always enabled by default
  ...
  const isOrganiser = currentParticipant?.role === 'organiser';
  ...
  useEffect(() => {
    ...
    const roomSubscription = supabase
      ...
      .on('postgres_changes', {
        ...
        // Auto-redirect when debate ends
        if (updatedRoom.status === 'ended' && room?.status === 'active') {
  ...
  }, [roomId, user, room?.status, navigate]);
      ...
      <div className="max-w-7xl mx-auto p-4">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-orange-500 rounded-lg p-6 text-white mb-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-4">
              {room.status === 'ended' && (
                <button
                  onClick={() => navigate(`/room/${roomId}/summary`)}
              ...
              <span className={`px-3 py-1 rounded-full text-sm font-medium ${
                room.status === 'active' ? 'bg-green-500/20 text-green-100' : 'bg-gray-500/20 text-gray-100'
              }`}>
                {room.status === 'active' ? '🔴 LIVE' : room.status === 'ended' ? '⏹️ ENDED' : '⏸️ WAITING'}
              </span>
            </div>
          </div>
        </div>

        {/* Auto-redirect message for ended debates */}
        {room.status === 'ended' && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
            <div className="flex items-center space-x-3">
              <FileText className="w-5 h-5 text-blue-600" />
              <div>
                <h4 className="text-sm font-medium text-blue-900">Debate Ended</h4>
                <p className="text-sm text-blue-700">
                  This debate has concluded. You'll be redirected to the summary page shortly, or click the button above to view it now.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div className="lg:col-span-3 space-y-6">
            {/* Voice Call Section for Anchors Only */}
            {currentParticipant && isAnchor && room.status === 'active' && (
              <div className="bg-white rounded-lg p-6 border-2 border-blue-200">
                <h3 className="text-lg font-semibold mb-4 flex items-center">
                  🎙️ Anchor Voice Communication
                </h3>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="text-sm text-gray-600">
                    {isInCall ? `🟢 Connected to ${connectedPeers.length} participants` : '⚫ Not connected'}
                  </div>
                </div>
              ...
              <div className="bg-white rounded-lg p-6 border-2 border-green-200">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    >
                      {isConnecting ? '🔄 Connecting...' : (
                        isInCall ? '🔇 Stop Listening' : '🔊 Listen to Debate'
                      )}
                    </button>
                    {isInCall && (
                      <button
                  ...
                  <div className="text-sm text-gray-600">
                    {isInCall ? `🔊 Listening to ${connectedPeers.length} anchors` : '⚫ Not connected'}
                  </div>
                </div>
                ...
                {isInCall && (
                  <div className="mt-4 text-sm text-gray-600">
                    ...
                    {connectedPeers.length > 0 && (
                      <p className="mt-1">Connected anchors: {connectedPeers.length}</p>
                    )}
                  </div>
                )}
            ...
            {/* AUTOMATIC Speech Recognition - For ALL participants */}
            {currentParticipant && room.status === 'active' && (
              <SpeechRecognitionControls
                roomId={roomId!}
                userId={user!.id}
                participantSide={currentParticipant.side}
                isAnchor={isAnchor}
                isEnabled={captionsEnabled}
                onToggleEnabled={() => setCaptionsEnabled(!captionsEnabled)}
              />
            )}

            {/* AUTOMATIC Live Captions for ALL participants */}
            {room.status === 'active' && captionsEnabled && (
              <CaptionsDisplay
                roomId={roomId!}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white rounded-lg p-6 border border-blue-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    onClick={() => vote('side_a')}
                    disabled={!user || userVote === 'side_a' || room.status !== 'active'}
              ...
              <div className="bg-white rounded-lg p-6 border border-orange-200">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <button
                    onClick={() => vote('side_b')}
                    disabled={!user || userVote === 'side_b' || room.status !== 'active'}
                    className={`px-4 py-2 rounded-lg font-medium transition-colors ${
                      userVote === 'side_b'
            ...
            <div className="bg-white rounded-lg p-6">
              ...
              {/* Message Input */}
              {user && participants.find(p => p.user_id === user.id) && room.status === 'active' ? (
                ...
                <div className="text-center text-gray-500 py-4">
                  {room.status !== 'active' ? (
                    <p>Debate has {room.status === 'ended' ? 'ended' : 'not started yet'}</p>
                  ) : !user ? (
                    <p>Sign in to participate in the discussion</p>
                  ) : (
                    <p>Join the debate to participate in the discussion</p>
                  )}
                  {!participants.find(p => p.user_id === user?.id) && user && room.status === 'active' && (
                    <button
          ...
          <div className="space-y-6">
            ...
            {/* Caption Status for Anchors */}
            {isAnchor && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-center space-x-2 mb-2">
                  <Subtitles className="w-4 h-4 text-blue-600" />
                  <h4 className="font-medium text-blue-900">Auto-Captions Active</h4>
                </div>
                <div className="text-xs text-blue-700 space-y-1">
                  <p>✅ Your speech is automatically converted to captions</p>
                  <p>🎯 Correctly attributed to your debate side</p>
                  <p>👥 Visible to all participants in real-time</p>
                  <p>🔄 Auto-restarts if interrupted</p>
                </div>
              </div>
            )}
          </div>
