...
export function OrganiseRoomPage() {
  ...
  const { generateSummary, loading: generatingSummary } = useDebateSummary();
  ...
  const startDebate = async () => {
    try {
      ...
      // Open live debate in new tab/window so organiser can manage from here
      window.open(`/room/${roomId}/live`, '_blank');
  ...
  const endDebate = async () => {
    const confirmed = window.confirm(
      'Are you sure you want to end this debate? This will automatically generate a summary and redirect all participants.'
    );
    if (!confirmed) return;
          ...
          <div className="bg-gradient-to-r from-blue-600 to-orange-500 p-6 text-white">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                ...
                {room.status === 'active' && (
                  <button
                    onClick={() => window.open(`/room/${roomId}/live`, '_blank')}
                    className="flex items-center space-x-2 px-4 py-2 bg-white/20 hover:bg-white/30 rounded-lg transition-colors"
                  >
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              {room.status === 'waiting' ? (
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={endDebate}
                  disabled={isEndingDebate || generatingSummary}
                  className="flex-1 flex items-center justify-center py-3 px-6 bg-gradient-to-r from-red-600 to-red-500 text-white rounded-lg hover:from-red-700 hover:to-red-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all font-semibold"
                >
                  {isEndingDebate || generatingSummary ? (
                    <>
                      <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin mr-2" />
                      {generatingSummary ? 'Generating Summary...' : 'Ending Debate...'}
                    </>
                  ) : (
                    <>
                      <Square className="w-5 h-5 mr-2" />
                      End Debate & Generate Summary
                    </>
                  )}
                </motion.button>
              ) : (
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => navigate(`/room/${roomId}/summary`)}
                  className="flex-1 flex items-center justify-center py-3 px-6 bg-gradient-to-r from-blue-600 to-orange-500 text-white rounded-lg hover:from-blue-700 hover:to-orange-600 transition-all font-semibold"
                >
            ...
            {room.status === 'active' && (
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="flex items-start space-x-3">
                  ...
                  <div>
                    <h4 className="text-sm font-medium text-blue-900">Debate is Live!</h4>
                    <p className="text-sm text-blue-700 mt-1">
                      The debate is currently active. When you end it, a comprehensive AI summary will be automatically generated and all participants will be redirected to view it.
                    </p>
                  </div>
                </div>
              </div>
